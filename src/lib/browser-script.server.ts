  ...
  try {
    ...
    const plan = await page.evaluate(function () {
      ...
      function valueFor(el, h) {
        ...
        if (type === "number" || has(h, "quantity", "amount", "age")) return "7";
        if (type === "date") return futureDate(7);
        if (type === "time") return "14:30";
        if (type === "datetime-local") return futureDate(7) + "T14:30";
        if (type === "search") return "test query";
        if (has(h, "first name", "firstname", "given", "fname")) return "Jordan";
        ...
        if (has(h, "name")) return "Jordan Ellis";
        if (el.tagName.toLowerCase() === "textarea" || has(h, "message", "comment", "enquiry", "inquiry", "details", "description")) {
      ...
      for (let i = 0; i < els.length; i++) {
        ...
        try {
          if (type === "checkbox") {
            ...
            const opts = Array.prototype.slice.call(el.options).filter(function (o) { return o.value !== "" && !o.disabled; });
            if (opts.length) {
              el.value = opts[0].value;
              el.dispatchEvent(new Event("change", { bubbles: true }));
              entry.value_used = opts[0].text.trim().slice(0, 60);
              entry.filled = true;
            } else {
