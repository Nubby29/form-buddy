...
export default async function ({ page, context }) {
  ...
  try {
    ...
    return { data: { ok: false, reason: "navigation_failed", message: String(e && e.message || e) }, type: "application/json" };
  }
  ...
  const plan = await page.evaluate(function () {
    ...
    if (!scope || best === 0) {
      ...
      if (!loose.length) return { ok: false, reason: "no_fields" };
      scope = document.body;
    }

    const els = fillable(scope);
    if (!els.length) return { ok: false, reason: "no_fields" };
    ...
    for (let i = 0; i < els.length; i++) {
      ...
      const h = hintOf(el);
      const entry = {
        label: labelText(el) || el.getAttribute("name") || "(unlabelled " + type + ")",
        selector: (el.getAttribute("name") ? "[name=" + el.getAttribute("name") + "]" : (el.id ? "#" + el.id : tag)),
        field_type: type,
      ...
      try {
        if (type === "checkbox") {
          ...
          const nm = el.getAttribute("name") || "anon";
    ...
    const box = scope === document.body ? (fields.length ? document.body : document.body) : scope;
    box.setAttribute("data-fta-form", "1");
    box.scrollIntoView({ block: "center" });

    let submitLabel = "";
    const btn = box.querySelector('button[type="submit"], input[type="submit"], button:not([type])') ||
    ...
    return {
      ...
      formSelector: scope.tagName.toLowerCase() + (scope.id ? "#" + scope.id : "") + (scope.getAttribute("name") ? "[name=" + scope.getAttribute("name") + "]" : ""),
    };
  });

  if (!plan.ok) {
    return { data: { ok: false, reason: plan.reason, title: title }, type: "application/json" };
  }
  ...
  if (!filledShot) filledShot = await page.screenshot({ encoding: "base64" });
  ...
  if (mode === "fill_submit") {
    ...
    if (submitted) {
      const after = await page.evaluate(function (beforeUrl) {
        ...
        if (pick) { pick.setAttribute("data-fta-result", "1"); focus = true; pick.scrollIntoView({ block: "center" }); }
        let snippet = "";
      ...
      if (!resultShot) resultShot = await page.screenshot({ encoding: "base64" });
    } else if (!resultShot) {
